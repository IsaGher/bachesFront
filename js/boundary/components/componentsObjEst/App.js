import ObjetoEstadoDataStore from '../../../control/ObjetoEstadoDataStore.js';

export default class App extends HTMLElement {
    constructor() {
      super();
      this.objetoEstado = new ObjetoEstadoDataStore();
      this._data = [];
    ...
    handleSubmit(event) {
      this.objetoEstado.modificar(event.detail);
        setTimeout(()=>{
          location.reload();
        },1500);
    }
    ...
    editCharacter(id) {
        let iD = Number(id);
        let valor = this._data.find(char => char.idObjetoEstado === iD);
